...
# --- 수정된 성능 평가 함수 (문제되는 계산 로직 삭제, 기본 데이터만 추출) ---
def check_performance(up_id):
    try:
        ...
        if not longforms: return False, 0
...
with st.form("search"):
    ...
    with c5: st.write("") # 문제의 소지가 될 수 있는 Min Efficiency 슬라이더 삭제
    ...
    for kw in keywords:
        try:
            ...
            for item in search['items']:
                ...
                is_ok, avg_v = check_performance(upid)
                ...
                if is_ok:
                    ...
                    final_list.append({
                        ...
                        "Subscribers": subs, 
                        "Avg Views": int(avg_v), 
                        # 문제되었던 Efficiency, Avg Engagement, Upload Frequency 모두 삭제 완료
                        "Email": email, 
